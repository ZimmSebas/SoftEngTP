...
:- dynamic(nopfcheck/0).      %default: pfcheck
...
set_default_control :-
    ...
    cond_retract(nopfcheck),         %set default value: pfcheck
...
write_atomic_constr(solved(C,_,_)) :-
    !,
    write(C).
...
setlog_tc(GoalString,OutConstrLst) :-      
  ...
  %catch(typecheck(Goal,VN),setlog_excpt(Msg),excpt_action(Msg)),    
  typecheck(Goal,VN),
...
get_groundsol_int(SFCgSet,[X|IntVars],SFCg) :-
  bb_inf_all([X|IntVars],[X|IntVars],Vertex),
...
setlog_command(pfcheck) :- !.
setlog_command(nopfcheck) :- !.
...
ssolve_command(pfcheck) :- !,         %% (re-)activate automatic application of pfun checks
    cond_retract(nopfcheck).
ssolve_command(nopfcheck) :-          %% deactivate automatic application of pfun checks
    nopfcheck,!.
ssolve_command(nopfcheck) :- !,
    assertz(nopfcheck).
...
sat_inters([inters(S1,S2,S3)|R1],R2,c,f) :-      % special case - inters(S1,{.../S1},S3), S3 and S1 var - smode(solver)
    %b_getval(smode,solver),
    compute_sols,
    var(S3),var(S1),
    nonvar(S2), S2 = _ with _,
...
sat_inters([inters(S1,S2,S3)|R1],R2,c,f) :-      % inters(S1,{...},S3), S3 and S1 var - smode(solver)
    %b_getval(smode,solver),
    compute_sols,
...
check_int_solutions(C,LIntVars) :-
    collect_integer(C,LIntVars),
    bb_inf_all(LIntVars,LIntVars,_).
